...
class SubForumPollCommentScreen extends StatefulWidget {
  final SubPollModel poll;

  const SubForumPollCommentScreen({Key? key, required this.poll})
      : super(key: key);

  @override
  _SubForumPollCommentScreenState createState() =>
      _SubForumPollCommentScreenState();
}

class _SubForumPollCommentScreenState extends State<SubForumPollCommentScreen> {
  ...
  void initState() {
    super.initState();
    _commentController =
        UniversalCommentController(targetType: CommentTargetType.subForumPoll);
  ...
  Future<void> _loadComments() async {
    print("Loading comments for subforum poll ID: ${widget.poll.id}");
    if (widget.poll.id?.isEmpty ?? true) {
      return;
    }
  ...
  // Get poll author info from Author object
  String _getPollAuthor() {
    final author = widget.poll.author;

    // First try profileName
    if (author.profileName != null && author.profileName!.isNotEmpty) {
  ...
  // Get poll author profile image from Author object
  ...
  // Build poll item with proper author extraction
        ...
        child: Column(
          ...
          children: [
            ...
            ...widget.poll.options.asMap().entries.map((entry) {
              ...
              final percentage = index < widget.poll.optionStats.length
                  ? widget.poll.optionStats[index].percentage
                  : 0.0;
